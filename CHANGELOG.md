...
**[버그 수정] 다운로드 시작 안 됨 + "시작부터 일시정지" 미작동 (develop 전용).**
*라벨 `0.5.1f`는 develop 반복 식별용 — PEP440상 유효 접미사(a/b/rc)가 아니므로 pyproject 버전에는 반영하지 않음.*

### Fixed (수정)
- 모든 토렌트가 `auto_managed=True`(순차 큐용)로 추가돼, 수동 `pause()`가 약 1틱 내 자동관리자에게 덮어써지던 문제. 이로 인해 **(1) "시작부터 일시정지"가 유지되지 않고, (2) 의도치 않게 resume된 토렌트가 `active_downloads=1` 슬롯을 점유해 뒤 토렌트가 다운로드를 시작하지 못함.**
- `pause()`: `auto_managed` 해제 후 pause → 정지 유지. `resume()`: `auto_managed` 복원 후 resume → 큐 재참여. `_load_resume_data()`: 수동 pause 상태는 재시작 후에도 보존(auto_managed 강제 복원 안 함).
- 회귀 테스트 4개 추가(paused 유지·resume 전이·정지 토렌트가 다른 토렌트 시작을 막지 않음·재시작 후 pause 유지). libtorrent 2.0.13 실핸들 상태로 검증. 전체 128 통과.
