...
**btdig형 검색 (실험적/알파).** 기본 비활성 + 명시적 법적 동의 게이트 뒤에서만 작동.

### Added (추가)
- **플러그인형 검색 구조** — `core/search/`: `SearchProvider`/`SearchResult` + 레지스트리(Qt 무의존), btdig 제공자(HTTP 질의 + `BeautifulSoup` 파싱 → magnet). 오류는 `SearchError`로 graceful.
- **법적 동의 게이트(D8)** — 검색 사용 전 (1)법적 위험 (2)다운로드 대상 S/W 라이선스 위반 가능성 (3)본인 전적 책임을 고지하고 체크+동의해야만 작동. 미동의 시 차단, `SearchSettings.consent_accepted`로 저장.
- **검색 UI** — `SearchDialog`(질의·결과 테이블·법적 고지 배너·다운로드 추가) + 설정 검색 탭. 기본 비활성.
- **결과 목록** — 제목·크기·**파일수**·**발견(age)**·출처 컬럼. **컬럼 헤더 클릭 정렬**(크기=바이트, 파일수=숫자, 발견=최근순, 미상값 마지막).
- **더블클릭 → 상세** — 전체 제목·크기·파일수·발견·infohash·**magnet 복사**·**파일 목록** + **"DHT에서 피어 확인"**(libtorrent DHT로 실 피어 수 조회 = 진짜 liveliness).
- **페이징("더 보기")** — btdig `p=N`으로 다음 페이지 append(중복 제거·카운트 표시). **기본 정렬 최근순(`order=2`)** — 살아있을 가능성 높은 최신 결과 우선.
- 의존성 `requests`·`beautifulsoup4`. 테스트: 파서(실제 btdig 마크업 픽스처)·동의 게이트·페이징·정렬·상세·DHT probe(mock). **라이브 네트워크 없음.**

### Notes (참고)
- **DHT 크롤러/인덱서는 만들지 않음** — btdig는 사용자 설정 base URL의 첫 제공자. HTTP 스크래핑이라 사이트 구조 변경에 취약(알파 품질).

## [0.5.2] - 2026-07-05

**다운로드 시작 버그 수정 + 진행 불가 상태 표시.**

### Fixed (수정)
- **다운로드가 시작되지 않고 "시작부터 일시정지"가 유지되지 않던 문제.** 모든 토렌트가 `auto_managed=True`(순차 큐용)로 추가돼 수동 `pause()`가 약 1틱 내 자동관리자에게 덮어써졌음 → (1) add-paused 미유지, (2) 의도치 않게 resume된 토렌트가 `active_downloads=1` 슬롯을 점유해 뒤 토렌트가 시작 못 함.
- `pause()`: `auto_managed` 해제 후 pause → 정지 유지. `resume()`: `auto_managed` 복원 후 resume → 큐 재참여. `_load_resume_data()`: 수동 pause 상태는 재시작 후에도 보존.
- 회귀 테스트 4개(paused 유지·resume 전이·정지 토렌트가 다른 토렌트 시작을 막지 않음·재시작 후 pause 유지). libtorrent 2.0.13 실핸들 검증.

### Added (추가)
- **"시더 없음" / "정체" 상태 표시.** 다운로드 중이지만 진행이 안 되는 토렌트를 구분: 연결 시드 0(스웜도 0/미상)이면 **"시더 없음"**, 속도 0인데 피어/시드가 있으면 **"정체"**. (사용자 관찰: 시드 없는 죽은 토렌트가 "다운로드중"으로만 떠 혼란.) `TorrentStatus`에 `num_complete`/`num_incomplete`(스웜 시드/피어, -1=미상) 추가, `models.py` 상태 라벨에서 판정. 단위 테스트 8개.
- 전체 136 테스트 통과, ruff clean.
