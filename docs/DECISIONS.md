...
## D8 — 검색 기능은 명시적 법적 동의 게이트 뒤에서만 사용 (실험적)
**결정:** btdig형 검색은 기본 비활성이며, 사용 전 사용자가 (1) 이 기능 사용이 법적으로
문제가 될 수 있고, (2) 다운로드하는 소프트웨어/콘텐츠의 **라이선스를 위반할 수 있으며**,
(3) 그에 대한 **모든 책임이 본인에게 있음**을 고지받고 체크박스+[동의]로 명시 동의해야만
작동한다. 미동의 시 검색·다운로드 추가를 차단. 동의 여부는 `SearchSettings.consent_accepted`로
config에 저장(매번 묻지 않음), 검색 UI에 법적 고지 배너 상시 노출.
**이유:** 검색은 법적으로 민감한 실험 기능(오너 지시). 사용자의 책임 있는 명시 동의를 사용
전에 강제해 리스크를 사용자에게 명확히 전달.
**되돌릴 수 있는가:** 예(게이트/설정 조정 가능). 실험 기능이라 develop에만 존재.
